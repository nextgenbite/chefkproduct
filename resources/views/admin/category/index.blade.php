            ...
            <table class="table center-aligned-table">
              ...
              <tbody>
                ...
                <tr class="">
                  ...
                  <td>{{$cat->category_dis}}</td>
                  @if ($cat->category_status ==1)
                      
                  <td><a href="{{URL::to('/admin/category/'.$cat->id.'/inactive')}} " class="badge badge-success">Active</a></td>
                  @else
                  <td><a href="{{URL::to('/admin/category/'.$cat->id.'/active')}} " class="badge badge-danger">Inactive</a></td>
                  @endif
        
                  <td><a href="{{URL::to('/admin/category/'.$cat->id.'/edit')}}" class="btn btn-primary btn-sm">Manage</a></td>
                  <td>
                    <form action="{{URL::to('/admin/category/'.$cat->id)}}" method="post">
                    @csrf
                    @method('DELETE')
