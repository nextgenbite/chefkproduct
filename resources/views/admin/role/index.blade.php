                ...
                <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="#"
                                class="ml-1 text-gray-700 hover:text-primary-600 md:ml-2 dark:text-gray-300 dark:hover:text-white">{{$title[0]}}</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
        ...
        <div class="sm:flex">
            <div class="items-center hidden mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700">
                <form class="lg:pr-3" action="#" method="GET">
                    <label for="data-search" class="sr-only">Search</label>
                    <div class="relative mt-1 lg:w-64 xl:w-96">
                        <input type="text" name="email" id="data-search"
            ...
            <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
                <button type="button" data-modal-target="add-data-modal" data-modal-toggle="add-data-modal"
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td class="p-4 space-x-2 whitespace-nowrap">
                                ...
                                <button type="button" data-modal-target="delete-data-modal"
                                    data-modal-toggle="delete-data-modal" data-id="{{ $item->id }}"
                                    class="delete-data inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd"
                                            d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z"
...
<div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full"
    ...
    <div class="relative w-full h-full max-w-2xl px-4 md:h-auto">
        <!-- Modal content -->
        <div class="relative max-h-[90dvh] overflow-y-auto bg-white rounded-lg shadow dark:bg-gray-800">
            <!-- Modal header -->
            <div class="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                ...
                <button type="button"
                    ...
                    data-modal-toggle="edit-data-modal">
    ...
    <div class="relative w-full h-full max-w-4xl px-4 md:h-auto">
        ...
        <div class="relative max-h-[90dvh] overflow-y-auto bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <form action="{{route('roles.store')}}" method="post">
                @include('admin.pertial.role_form')
                <!-- Modal footer -->
                <div class="items-center p-6 border-t border-gray-200 rounded-b dark:border-gray-700">
                    <button
                        class="btn-submit text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
    ...
    <div class="relative w-full h-full max-w-md px-4 md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <div class="flex justify-end p-2">
                <button type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-700 dark:hover:text-white"
                    data-modal-hide="delete-data-modal">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd"
                            d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                            clip-rule="evenodd"></path>
...
<script>
   ...
   function handleSelectAllChange() {
    ...
    document.addEventListener('change', function (event) {
        if (event.target && event.target.id === 'checkbox-all') {
            handleSelectAllChange.call(event.target);
        }
    });
    // Use event delegation for group checkboxes
    document.addEventListener('change', function (event) {
        if (event.target.classList.contains('group-checkbox')) {
            const groupCheckbox = event.target;
            const group = groupCheckbox.id.replace('checkbox-', '');
...
$(document).ready(function() {
    ...
    // $(document).on('click', '.edit-role', function () {alert('ok')})
    $('.edit-data').on('click', function() {
        var roleId = $(this).data('id');
        $('#editForm').attr('action', "{{url('admin/roles')}}"+'/' + roleId);
        $.get("{{ url('/admin/roles') }}/" + roleId, function (data) {
            $('#editFormBody').html(data.html)
        })
    });
    ...
    $('.delete-data').on('click', function() {
        var roleId = $(this).data('id');
        $('#deleteRoleForm').attr('action', "{{url('admin/roles')}}"+'/' + roleId);
