...
<div
    ...
    <a href="{{Route('product.view',$product->slug)}}" class="relative ">
        ...
        <img src="{{asset('/images/loader.svg')}}" width="40" data-src="{{ asset($product->thumbnail ?? 'images/no-image.png') }}" alt="{{ $product->title }}" class="thumb lazy " />
    ...
    {{-- <button  @click="addToCart(product)" class="block w-full py-1 text-center sm:text-xs text-white bg-primary-light border border-primary-light rounded-b font-medium hover:bg-transparent hover:text-primary-light transition ease-in duration-500" >
          Buy Now
     </button>  --}}
