        ...
        @if (session()->has('cart') && isset(session('cart')['data']))
            ...
            @foreach (session('cart')['data'] as $item)
                ...
                <a title="{{ $item['title'] }}" href="#"
                    class="px-6 py-3 flex items-center justify-between hover:bg-gray-100 transition ">
                    <img alt="image" src="{{ asset($item['thumbnail']) }}" alt="category thumb"
                        class="md:w-10 md:ml-2 max-w-full max-h-full rounded object-contain " />
                    <div>
                        <div class=" text-gray-700 text-sm font-semibold "> {{ $item['title'] }}</div>
                        <div class=" text-gray-700 text-sm font-semibold ">
                            {{ $item['quantity'] }}&nbspx&nbsp{{ formatcurrency($item['price']) }}</div>
                    </div>
